...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Get all users — no index needed, filter in code ──
    if (action === 'get_users') {
      ...
      const promises = usersSnap.docs.map(async (userDoc) => {
        try {
          ...
          if (profileDoc.exists) {
            ...
            if (data.email) users.push({ uid: userDoc.id, ...data });
          }
        } catch (e) {
          console.warn('Could not fetch profile for', userDoc.id, e.message);
        }
      });
      ...
      users.sort((a, b) => {
        const ta = a.last_active?.toMillis?.() || 0;
        const tb = b.last_active?.toMillis?.() || 0;
        return tb - ta;
      });
    ...
    if (action === 'revoke_promax') {
      ...
      await db.collection('users').doc(targetUid)
        .collection('profile').doc('data').update({
